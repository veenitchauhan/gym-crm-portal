...
export default function MemberDashboard({member,upcomingBookings,availableSessions,recentAttendance,payments,club}:Props) {
    ...
    return <><Head title={`My membership · ${gymName}`}/><div className="member-app">
        ...
        <main className="member-content"><section className="member-hero"><div><span className="eyebrow">MY CLUB · {gymName.toUpperCase()}</span><h1>Welcome, {firstName}.</h1><p>Your membership, visits, payments, and bookings are connected here.</p></div><div className="member-streak"><UserRoundCheck/><strong>{member.visitsThisWeek}</strong><span>visits this week</span></div></section>
            <section className="member-stats"><article><span className="member-stat-icon purple"><Activity/></span><div><small>Visits this month</small><strong>{member.visitsThisMonth}</strong><p>{member.visitsThisMonth?'Attendance recorded':'No visits yet'}</p></div></article><article><span className="member-stat-icon green"><Clock3/></span><div><small>Training time</small><strong>{trainingHours}h</strong><p>Completed visits this month</p></div></article><article><span className="member-stat-icon orange"><CalendarDays/></span><div><small>Upcoming bookings</small><strong>{upcomingBookings.length}</strong><p>{upcomingBookings.length?'Sessions reserved':'Nothing booked'}</p></div></article></section>
